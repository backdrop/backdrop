(function ($) {
  ...
  Backdrop.behaviors.adminBar = {
    attach: function (context, settings) {
      ...
      if (!$adminBar.length && settings.admin_bar.hash) {
        Backdrop.adminBar.getCache(
          settings.admin_bar.hash,
          function (response) {
  ...
  Backdrop.adminBar.getCache = function (hash, onSuccess) {
    ...
    if (!Backdrop.adminBar.ajaxRequest) {
      Backdrop.adminBar.ajaxRequest = $.ajax({
        ...
        url: Backdrop.settings.admin_bar.basePath.replace(
          /admin_bar/,
          "js/admin_bar/cache/" + hash
        ),
        success: [
          function (response) {
   ...
   */
  if (
    Backdrop.settings.admin_bar.hash &&
    Backdrop.settings.admin_bar.basePath
  ) {
   ...
   */
  Backdrop.adminBar.behaviors.adminBarActiveTrail = function (
    context,
    settings,
    $adminBar
  ) {
    if (settings.admin_bar.activeTrail) {
      $adminBar
        .find(
          '#admin-bar-menu > li > ul > li > a[href="' +
            settings.admin_bar.activeTrail +
            '"]'
        )
   ...
   */
  Backdrop.adminBar.behaviors.positionFixed = function (
    context,
    settings,
    $adminBar
  ) {
   ...
   */
  Backdrop.adminBar.behaviors.replacements = function (
    context,
    settings,
    $adminBar
  ) {
   ...
   */
  Backdrop.adminBar.behaviors.destination = function (
    context,
    settings,
    $adminBar
  ) {
    if (settings.admin_bar.destination) {
      $("a.admin-bar-destination", $adminBar).each(function () {
        this.search +=
          (!this.search.length ? "?" : "&") +
          Backdrop.settings.admin_bar.destination;
      });
    }
  };
   ...
   */
  Backdrop.adminBar.behaviors.collapseWidth = function (
    context,
    settings,
    $adminBar
  ) {
  ...
  Backdrop.adminBar.behaviors.hover = function (context, settings, $adminBar) {
    ...
    $adminBar.on(
      ...
      "li.expandable > a, li.expandable > span",
      function (e) {
        ...
        if (e.type === "touchstart") {
          ...
          needsExpanding =
            ...
            !$(this).siblings("ul").hasClass("expanded");
        }
  ...
  Backdrop.adminBar.behaviors.search = function (context, settings, $adminBar) {
    ...
    var $results = $('<div class="admin-bar-search-results" />').insertAfter(
      $input.parent()
    );
      ...
      if (value !== needle || e.type === "focus") {
        ...
        if (!links && needle.length >= needleMinLength) {
          links = buildSearchIndex(
            $adminBar.find(
              "#admin-bar-menu .dropdown li:not(.admin-bar-action, .admin-bar-action li) > a"
            )
          );
        }
    ...
    function buildResultsList(matches) {
      ...
      $.each(matches, function () {
        ...
        var $result = $(
          '<li><a href="' + $element.attr("href") + '"> </a></li>'
        );
