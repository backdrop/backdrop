 ...
 *   - alias: (optional) A string with a new or alternative machine name (key)
 *     for an old permission. Use this to create permission "redirects" from a
 *     previous permission to a new/renamed one. Any user_access() check against
 *     the permission specifying an alias, will actually trigger a check against
 *     the alias. This is useful for example when renaming the machine names of
 *     permissions to replace spaces with underscores.
 *     When the 'alias' key is specified for a permission, the 'title' key is
 *     ignored (so can be omitted).
 *   - description: (optional) A description of what the permission does. This
 *     should be wrapped in the t() function, so that it can be translated.
 *   - restrict access: (optional - defaults to FALSE) A boolean which can be
 *     set to TRUE to indicate that site administrators should restrict access
 *     to this permission to trusted users. This should be used for permissions
 *     that have inherent security risks across a variety of potential use cases
 *     (for example, the "administer filters" and "bypass node access"
 *     permissions provided by Backdrop core). When set to TRUE, a standard
 *     warning message defined in user_generate_form_permissions() and output
 *     via theme_user_permission_description() will be associated with the
 *     permission, and displayed with it on the permission administration page.
 *   - deprecated: (optional - defaults to FALSE) A boolean which can be set to
 *     TRUE to indicate that this permission has been deprecated. This should be
 *     used for backwards compatibility purposes, when a permission is removed,
 *     renamed, or replaced by another permission. When set to TRUE, a standard
 *     warning message defined in user_generate_form_permissions() and output
 *     via theme_user_permission_description() will be associated with the
 *     permission, and displayed with it on the permission administration page.
 *   - warning: (optional) A translated warning message to be displayed for this
 *     permission on the permissions administration page. It overrides the
 *     default warnings that are generated by setting the 'restrict access' or
 *     'deprecated' keys to TRUE.
 *     This should rarely be used, since it is important for all permissions to
 *     have clear security or deprecation warnings, that are consistent across
 *     the site. To provide any information that is specific to the permission
 *     you are defining, use the 'description' key instead.
 *
...
function hook_permission() {
  return array(
    ...
    'old permission' => array(
      'title' => t('Some old permission'),
      'deprecated' => TRUE,
      'description' => t('This permission has been removed. See the <a href="@change_record">change record</a> for details.', array('@change_record' => 'https://link.to.some/change_record')),
    ),
  );
}
