...
(function ($) {
  Backdrop.color = {
    ...
    callback: function(context, settings, form, farb, height, width) {
      ...
      function checkTab() {
        if($('#edit-main-menu-tabs-no-tabs').is(':checked')) { updateTabs('none', 'none', 0); }
        if($('#edit-main-menu-tabs-rounded-tabs').is(':checked')) { updateTabs('rgba(255, 255, 255, 0.7)', '0 1px #eee', '8px'); }
        if($('#edit-main-menu-tabs-square-tabs').is(':checked')) { updateTabs('rgba(255, 255, 255, 0.7)', '0 1px #eee', 0); }
      }
      
      function updateTabs(bg, shadow, radius) {
        $('#preview #preview-main-menu-links a').css({'background':bg, 'text-shadow':shadow, 'border-top-left-radius':radius, 'border-top-right-radius':radius});
      }
