 ...
 *
 * IMPORTANT!: After finishing the upgrade, be sure to edit this file again and
 ...
 * contents of a file outside your docroot that is never saved together with any
 * backups of your Backdrop files and database.
 *
 * Example:
 * @code
 ...
 *
 * Since the HTTP Host header can be set by the user making the request, it is
 * possible for malicious users to override it and create an attack vector. To
 * protect against these sort of attacks, Backdrop supports checking a list of
 * trusted hosts.
 *
 ...
 *
 * I.e. the time from the user's last visit to the active session may be deleted
 * by the session garbage collector. When a session is deleted, authenticated
 * users are logged out, and the contents of the user's $_SESSION variable is
 * discarded.
 */
 ...
 * Maintenance theme.
 *
 * A custom theme can be set for the offline page. This applies when the site is
 * explicitly set to maintenance mode through the administration page or when
 ...
 * visited sites and may also provide other site caching, security, or
 * encryption benefits. In an environment where Backdrop is behind a reverse
 * proxy, the real IP address of the client should be determined such that the
 * correct client IP address is available to Backdrop's logging and access
 * management systems. In the most simple scenario, the proxy server will add an
 * X-Forwarded-For header to the request that contains the client IP address.
 * However, HTTP headers are vulnerable to spoofing, where a malicious client
 * could bypass restrictions by setting the X-Forwarded-For header directly.
 * Therefore, Backdrop's proxy configuration requires the IP addresses of all
 * remote proxies to be specified in $settings['reverse_proxy_addresses'] to
 * work correctly.
 *
 * Enable this setting to get Backdrop to determine the client IP from the
 * X-Forwarded-For header (or $settings['reverse_proxy_header'] if set). If you
 * are unsure about this setting, do not have a reverse proxy, or Backdrop
 * operates in a shared hosting environment, this setting should remain
 * commented out.
 *
 * In order for this setting to be used, you must specify every possible reverse
 * proxy IP address in $settings['reverse_proxy_addresses']. If a complete list
 * of reverse proxies is not available in your environment (for example, if you
 * use a CDN), then you may set the $_SERVER['REMOTE_ADDR'] variable directly in
 * settings.php. Be aware, however, that it is likely that this would allow IP
 * address spoofing unless more advanced precautions are taken.
 */
// $settings['reverse_proxy'] = TRUE;

/**
 * Reverse proxy IP address list.
 *
 * Specify every reverse proxy IP address in your environment. This setting is
 * required if the $settings['reverse_proxy'] setting above is enabled and set
 * to TRUE.
 */
// $settings['reverse_proxy_addresses'] = array('a.b.c.d', ...);

/**
 * Custom header for reverse proxy client IP.
 *
 ...
 * views. This tells a HTTP proxy that it may return a page from its local cache
 * without contacting the web server, if the user sends the same "Cookie" header
 * as the user who originally requested the cached page. Without "Vary: Cookie",
 * authenticated users would also be served the anonymous page from the cache.
 * If the site has mostly anonymous visitors except a few known editors or
 * administrators, the "Vary" header can be omitted. This allows for better
 * caching in HTTP proxies (including reverse proxies), i.e. even if clients
 * send different cookies, they still get content served from the cache.
 ...
 *
 * The options below return a simple, fast 404 page for URLs matching a specific
 * pattern:
 ...
 * site administrators to securely install missing updates for the site directly
 * through the web user interface. On securely-configured servers, the Update
 * Manager will require the administrator to provide SSH or FTP credentials
 * before allowing the installation to proceed; this allows the site to update
 * the new files as the user who owns all the Backdrop files, instead of as the
 * user the web server is running as. On servers where the web server user is
 * itself the owner of the Backdrop files, the administrator will not be
 * prompted for SSH or FTP credentials (note that these server setups are common
 * on shared hosting, but are inherently insecure).
 *
 * Some sites might wish to disable the above functionality, and only update the
 * code directly via SSH or FTP themselves. This setting completely disables all
 * functionality related to these authorized file operations.
 *
 * Uncomment the line below to disable authorized operations.
 */
 ...
 * within the files stored in the $config_directories variable above. This can
 * be useful to store per-environment values or sensitive data that is
 * undesirable to store in the config storage.
 *
 ...
 * private, but other schemes, such as "https", "s3", "example", or others, can
 * be either public or private depending on the site. By default, they are
 ...
 * for the scheme, or a different module that decides to make the scheme public.
 * However, in cases where a site needs to make a scheme public, but is unable
 * to add code in a module to do so, the scheme may be added to this variable,
 * the result of which is that system_file_download() grants public access to
 * all files within that scheme.
 */
 ...
 *
 * These variables allow opting out of this behavior. Careful consideration of
